...
from Model.train_eval import tr_remove, eval_fn
...
def run(
    ...
    with console.status("Initializing Model") as status:
        ...
        _, optim_, _ = privacy_engine.make_private_with_epsilon(
            module=model,
            optimizer=optim,
            data_loader=deepcopy(tr_loader),
            epochs=args.epochs,
            target_epsilon=args.epsilon,
            target_delta=1e-5,
            max_grad_norm=args.cgrad,
        )
        args.ns = optim_.noise_multiplier
        del model
        del optim
        del optim_
        model = init_model(args=args).to(device)
        optim = torch.optim.Adam(model.parameters(), lr=args.lr)
        console.log(f"Using sigma={args.ns} and C={args.cgrad}")
